...
template <typename T>
void BinaryTree<T>::printPaths(const Node * subRoot, vector<T> processed)const
{
	if(subRoot==NULL)
	return;
	processed.push_back(subRoot->elem);
	...
	cout<<" "<<processed.front();
	processed.erase(processed.begin());
	}
	cout<<endl;
	}
	else{
	printPaths(subRoot->left, processed);
	printPaths(subRoot->right, processed);
...
int BinaryTree<T>::sumDistances(const Node * subRoot, int sum) const{
	...
	return sum;
	if(subRoot->left==NULL)
	return sum+sumDistances(subRoot->right, sum+1);
	if(subRoot->right==NULL)
	return sum+sumDistances(subRoot->left, sum+1);
