...
class KDTree
{
    public:
         ...
         * lowerest bounding hyperrectangle, then the corresponding node is
    ...
    private:
        ...
        int getPrintData(int lower, int high) const;

        /** Helper function for grading */
        void printTree(int lower, int high, std::vector<std::string> & output,
                int left, int top, int width, int currd) const;
		void buildTree(int lower, int upper, int dim);
		void partition(int lower, int upper, int curDim, int k);
		Point<Dim> walkThrough(const Point<Dim> &target, const Point<Dim> &ret, int lower, int upper, int curDim) const;
        /**
